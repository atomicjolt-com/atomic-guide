...
async function handleRegistrationSubmission(
  ...
  try {
    ...
    if (registrationData.registrationToken) {
      ...
      return await completeRegistrationWithPlatform(
        registrationData,
        toolConfig,
        env
      );
...
export function createToolConfigurationResponse(
...
export async function storePlatformConfiguration(
  registrationData: RegistrationData,
  env: Env
): Promise<void> {
  ...
  await env.PLATFORMS.put(
    platformConfig.platform_id,
    JSON.stringify(platformConfig)
  );
  
  // Also store by client_id for quick lookup
  await env.PLATFORMS.put(
    `client:${platformConfig.client_id}`,
    JSON.stringify(platformConfig)
  );
}
```
...
export class CanvasRegistrationHandler {
  async handleCanvasRegistration(
    registrationData: RegistrationData
  ): Promise<CanvasRegistrationResponse> {
...
export class MoodleRegistrationHandler {
  async handleMoodleRegistration(
    registrationData: RegistrationData
  ): Promise<MoodleRegistrationResponse> {
...
<head>
    ...
    <style>
        ...
        .platform-info { background: #f5f5f5; padding: 15px; margin-bottom: 20px; }
    </style>
</head>
<body>
    ...
    <form id="registrationForm" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="url" name="platform_url" id="platformUrl" required 
        ...
        <div class="form-group">
            ...
            <input type="text" name="institution_name" id="institutionName" required 
        ...
        <div class="form-group" id="deploymentGroup">
            ...
            <input type="text" name="deployment_id" id="deploymentId" 
        ...
        <div class="form-group">
            ...
            <button type="button" class="button" onclick="downloadConfig()" 
...
<head>
    ...
    <style>
        ...
        .config-details { background: #f8f9fa; padding: 20px; margin: 20px 0; text-align: left; }
        .button { background: #007cba; color: white; padding: 10px 20px; border: none; text-decoration: none; display: inline-block; margin: 10px; }
    </style>
</head>
<body>
...
export async function validateRegistrationToken(
  ...
  try {
    ...
    // Verify token claims
    return (
      decoded.aud === 'https://guide.yourdomain.com' &&
      decoded.exp > Date.now() / 1000 &&
      decoded.iss === platformUrl
    );
  } catch (error) {
