...
export async function handleDeepLinking(c: Context<{ Bindings: Env }>): Promise<Response> {
  try {
    ...
    const assessmentItems = content_items.filter((item) => item.custom?.assessment_config && item.lineItem);
    ...
    const signedJWT = await signDeepLinkResponse(validatedAssessments, assessmentIds, c.env);
 ...
 */
async function storeAssessments(assessments: any[], db: D1Database): Promise<string[]> {
  const ids: string[] = [];
 ...
 */
async function signDeepLinkResponse(assessments: any[], assessmentIds: string[], env: Env): Promise<string> {
