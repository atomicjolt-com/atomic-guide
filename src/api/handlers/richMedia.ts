...
import { PromptBuilder, type PromptContext } from '../../services/PromptBuilder';
  ...
  try {
    ...
    switch (media_type) {
      case 'latex':
        ...
        sanitizedContent = serverDOMPurify.sanitize(content, {
          ALLOWED_TAGS: [],
          ALLOWED_ATTR: []
        });
        ...
        if (nestedBraces > 50) { // Configurable limit
          return c.json({ 
      ...
      case 'code':
        ...
        if (lineCount > 500) { // AC requirement: 500 line limit
          return c.json({
      ...
      case 'diagram':
        sanitizedContent = serverDOMPurify.sanitize(content, {
          ...
          ALLOWED_ATTR: [
            'viewBox', 'width', 'height', 'd', 'cx', 'cy', 'r', 
    ...
    if (cached) {
      ...
      await c.env.DB.prepare(`
        ...
        WHERE content_hash = ?
      `).bind(new Date().toISOString(), contentHash).run();
  ...
  try {
    ...
    const validPreferences = {
      ...
      math_notation_style: ['latex', 'ascii', 'spoken'].includes(preferences.math_notation_style) 
        ? preferences.math_notation_style : 'latex',
      code_highlight_theme: ['light', 'dark'].includes(preferences.code_highlight_theme) 
        ? preferences.code_highlight_theme : 'light',
      diagram_complexity: ['simple', 'detailed'].includes(preferences.diagram_complexity) 
        ? preferences.diagram_complexity : 'detailed',
      bandwidth_preference: ['high', 'medium', 'low'].includes(preferences.bandwidth_preference) 
        ? preferences.bandwidth_preference : 'high'
    };
...
function detectProgrammingLanguage(code: string): string {
  const patterns = [
    ...
    { lang: 'sql', pattern: /^(SELECT |FROM |WHERE |INSERT |UPDATE |DELETE |CREATE TABLE)/mi },
    { lang: 'html', pattern: /^(<html|<!DOCTYPE|<div|<span|<p>)/mi },
