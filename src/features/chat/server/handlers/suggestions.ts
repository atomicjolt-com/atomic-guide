...
app.post('/feedback', async (c) => {
  try {
    ...
    // Record feedback
    await suggestionEngine.recordSuggestionFeedback(suggestionId, action, feedback, followupBehavior, db);
    ...
    if (details && feedback) {
      ...
      await db
        ...
        .bind(`feedback-${suggestionId}-${Date.now()}`, suggestionId, feedback, details, followupBehavior || null)
        .run();
    }
...
app.post('/dismiss', async (c) => {
  try {
    ...
    await db
      .prepare(dismissalQuery)
      .bind(`dismiss-${suggestionId}`, reason || 'user_dismissed', JSON.stringify(contextData || {}), suggestionId)
      .run();
...
app.put('/preferences', async (c) => {
  try {
    const { frequency, patternTrackingEnabled, preferredSuggestionTypes, interruptionThreshold, escalationConsent, cooldownMinutes } =
      await c.req.json();
    ...
    const result = await db.prepare(updateQuery).bind(JSON.stringify(preferences), learnerId, tenantId).run();
 ...
 */
async function _getStudentSuggestionStats(db: any, tenantId: string, learnerId: string, startDate: Date): Promise<any> {
  const statsQuery = `
    SELECT 
  ...
  const results = await db.prepare(statsQuery).bind(tenantId, learnerId, startDate.toISOString()).all();
  ...
  return {
    ...
    avg_effectiveness_score: results.reduce((sum: number, row: any) => sum + (row.avg_effectiveness || 0), 0) / results.length || 0,
  };
}
 ...
 */
async function _getLearningProgressStats(db: any, tenantId: string, learnerId: string, startDate: Date): Promise<any> {
  const progressQuery = `
    SELECT 
      pattern_type,
      pattern_strength,
  ...
  const patterns = await db.prepare(progressQuery).bind(learnerId, startDate.toISOString()).all();
 ...
 */
async function _getCourseSuggestionStats(db: any, tenantId: string, startDate: Date): Promise<any> {
  const courseQuery = `
    SELECT 
  ...
  const result = await db.prepare(courseQuery).bind(tenantId, startDate.toISOString()).first();

  return {
    active_learners: result?.active_learners || 0,
    total_suggestions: result?.total_suggestions || 0,
    acceptance_rate: result?.total_suggestions > 0 ? (result.accepted_suggestions || 0) / result.total_suggestions : 0,
 ...
 */
async function _getPatternAnalytics(db: any, tenantId: string, startDate: Date): Promise<any> {
  const patternQuery = `
    SELECT 
      suggestion_type,
      COUNT(*) as frequency,
  ...
  const patterns = await db.prepare(patternQuery).bind(tenantId, startDate.toISOString()).all();
  ...
  return {
    ...
    most_effective_pattern:
      patterns.reduce((best: any, current: any) => {
