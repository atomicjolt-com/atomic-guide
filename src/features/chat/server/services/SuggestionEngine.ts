...
export class SuggestionEngine {
  ...
  private async getLearningPatternAnalyzer() {
    if (!this.learningPatternAnalyzer) {
      const { LearningPatternAnalyzer } = await import('@features/dashboard/server/services/LearningPatternAnalyzer');
   ...
   */
  async generateProactiveSuggestions(request: ProactiveSuggestionRequest, db: any): Promise<any[]> {
    ...
    try {
      ...
      const analysis = await patternAnalyzer.analyzeConversation(request.messages, request.learnerProfile, request.context);
