...
export const SuggestionAnalytics: React.FC<SuggestionAnalyticsProps> = ({ role = 'student', timeframe = 'week', className = '' }) => {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-2">
            ...
            <h4 className="font-medium text-gray-900 dark:text-white">{patternInfo.label}</h4>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            ...
            <span className="font-semibold text-gray-900 dark:text-white">{pattern.frequency}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-600 dark:text-gray-300">Acceptance Rate</span>
            <span className="font-semibold text-gray-900 dark:text-white">{formatPercentage(pattern.acceptance_rate)}</span>
          </div>
        </div>
        ...
        <div className="mt-3">
          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
            <div className={`h-2 rounded-full bg-${patternInfo.color}-500`} style={{ width: formatPercentage(pattern.acceptance_rate) }} />
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        ...
        {trend && (
          ...
          >
            <span className="text-sm">{trend === 'up' ? '↗️' : trend === 'down' ? '↘️' : '➡️'}</span>
            <span className="text-sm font-medium">{trend === 'up' ? 'Improving' : trend === 'down' ? 'Needs attention' : 'Stable'}</span>
          </div>
        )}
      </div>
    );
    ...
    <div className={`suggestion-analytics ${className} space-y-6`}>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-300 text-sm">{TIMEFRAME_LABELS[selectedTimeframe as keyof typeof TIMEFRAME_LABELS]}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard title="Total Suggestions" value={data.suggestion_stats.total_shown} icon="💡" subtitle="Shown during timeframe" />

        <MetricCard
          title="Acceptance Rate"
          value={data.suggestion_stats.acceptance_rate}
          icon="✅"
          trend={data.suggestion_stats.acceptance_rate > 0.4 ? 'up' : data.suggestion_stats.acceptance_rate > 0.2 ? 'stable' : 'down'}
        />
        ...
        <MetricCard
          ...
          icon="👍"
          trend={data.suggestion_stats.helpful_rate > 0.6 ? 'up' : data.suggestion_stats.helpful_rate > 0.4 ? 'stable' : 'down'}
        />
        ...
        <MetricCard
          ...
          trend={
            data.learning_progress.suggestion_effectiveness > 0.7
              ? 'up'
              : data.learning_progress.suggestion_effectiveness > 0.5
                ? 'stable'
                : 'down'
          }
        />
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Most Common Patterns</h3>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Learning Improvements</h3>
            ...
            <div className="space-y-4">
              {data.learning_progress.pattern_improvements.map((improvement) => (
                <div key={improvement.pattern} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    <span className={getProgressColor(improvement.trend)}>{getProgressIcon(improvement.trend)}</span>
                    <span className={`text-sm font-medium ${getProgressColor(improvement.trend)}`}>{improvement.trend}</span>
                  </div>
                </div>
              ))}
              ...
              {data.learning_progress.optimal_timing_learned && (
                <div className="mt-4 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm font-medium text-green-800 dark:text-green-200">Optimal suggestion timing learned!</span>
                  </div>
      ...
      <div className="text-center text-xs text-gray-500 dark:text-gray-400 pt-4 border-t border-gray-200 dark:border-gray-700">
        ...
        <p className="mt-1">
          Based on {data.learning_progress.total_patterns_analyzed} conversation
          {data.learning_progress.total_patterns_analyzed !== 1 ? 's' : ''} analyzed
        </p>
      </div>
    </div>
  );
