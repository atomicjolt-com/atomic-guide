...
describe('StruggleDetectorDO', () => {
  ...
  describe('fetch handler', () => {
    ...
    it('should handle get_realtime_status requests', async () => {
      const mockSessionData = {
        ...
        recentPatterns: [{ studentId: 'student-1', pattern: 'repeated_errors', severity: 0.7 }],
        alertsGenerated: 2,
      };
  ...
  describe('alert generation', () => {
    it('should generate instructor alert for high severity struggles', async () => {
      ...
      await struggleDetector.generateAlert('student-1', 'course-1', highSeverityPattern);
    ...
    it('should queue intervention for medium severity struggles', async () => {
      ...
      await struggleDetector.generateAlert('student-1', 'course-1', mediumSeverityPattern);
  ...
  describe('real-time WebSocket communication', () => {
    it('should broadcast struggle patterns to connected clients', () => {
      ...
      expect(mockWebSocket1.send).toHaveBeenCalledWith(JSON.stringify({ type: 'struggle_detected', data: pattern }));
      expect(mockWebSocket2.send).toHaveBeenCalledWith(JSON.stringify({ type: 'struggle_detected', data: pattern }));
    });
  ...
  describe('alarm handler for periodic analysis', () => {
    ...
    it('should clean old interaction data during alarm', async () => {
      ...
      const oldInteractions = [{ type: 'assessment_attempt', timestamp: oldDate.toISOString() }];

      mockStorage.list.mockResolvedValue(new Map([['student-1', oldInteractions]]));
