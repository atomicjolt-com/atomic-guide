...
import type { PerformanceData, PrivacySettings } from '@features/dashboard/shared/types';
...
describe('PrivacyPreservingAnalytics', () => {
  ...
  describe('anonymizeBenchmarkData', () => {
    ...
    it('should apply stronger noise with smaller epsilon', () => {
      const studentData: PerformanceData[] = [{ studentId: 'student-1', score: 80, timeSpent: 3600 }];
  ...
  describe('validatePrivacyCompliance', () => {
    it('should validate consent before analytics operations', async () => {
      ...
      const isCompliant = await service.validatePrivacyCompliance('tenant-1', 'student-1', 'performance_tracking');
    ...
    it('should handle missing consent records', async () => {
      ...
      const isCompliant = await service.validatePrivacyCompliance('tenant-1', 'student-1', 'any_operation');
    ...
    it('should validate FERPA compliance for instructor access', async () => {
      ...
      const isCompliant = await service.validateFERPACompliance('tenant-1', 'instructor-1', 'student-1');
  ...
  describe('auditDataAccess', () => {
    ...
    it('should detect suspicious access patterns', async () => {
      ...
      const isSuspicious = await service.detectSuspiciousAccess('tenant-1', 'suspicious-user');
  ...
  describe('managementConsentWithdrawal', () => {
    it('should handle consent withdrawal and data deletion', async () => {
      mockD1Database.run.mockResolvedValue({ success: true });
      mockD1Database.batch.mockResolvedValue([{ success: true }, { success: true }, { success: true }]);

      await service.handleConsentWithdrawal('tenant-1', 'student-1', ['performance_tracking', 'data_sharing']);
    ...
    it('should maintain audit trail of consent changes', async () => {
      ...
      await service.updateConsentStatus('tenant-1', 'student-1', {
        analytics_enabled: false,
        performance_tracking: false,
        data_sharing: false,
      });

      const auditQuery = mockD1Database.prepare.mock.calls.find((call) => call[0].includes('consent_audit'));
    ...
    it('should support granular consent options', async () => {
      ...
      await service.updateConsentStatus('tenant-1', 'student-1', consentOptions);
  ...
  describe('generatePrivacyReport', () => {
    it('should generate comprehensive privacy report for student', async () => {
      const mockData = {
        ...
        audit_logs: {
          results: [{ operation: 'view_performance', timestamp: new Date().toISOString() }],
        },
      };
      ...
      const report = await service.generatePrivacyReport('tenant-1', 'student-1');
    ...
    it('should support data export in GDPR format', async () => {
      ...
      const exportData = await service.exportUserDataGDPR('tenant-1', 'student-1');
  ...
  describe('differentialPrivacyMechanisms', () => {
    ...
    it('should apply randomized response for binary data', () => {
      ...
      const responses = Array(10)
        .fill(null)
        .map(() => service.randomizedResponse(truthfulAnswer, probability));
