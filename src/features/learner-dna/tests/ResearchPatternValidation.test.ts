...
describe('Research Pattern Validation', () => {
  ...
  describe('Cognitive Load Theory Validation (Sweller)', () => {
    ...
    it('should show cognitive load effects on performance', () => {
      ...
      const highCapacitySessions = largeDataset.sessions.filter(s => 
        highCapacity.some(p => p.studentId === s.studentId)
      );
      const lowCapacitySessions = largeDataset.sessions.filter(s =>
        lowCapacity.some(p => p.studentId === s.studentId)
      );
      
      // Calculate average accuracy
      const highCapacityAccuracy = highCapacitySessions.reduce((sum, s) => 
        sum + (s.correctAnswers / Math.max(1, s.questionsAnswered)), 0
      ) / highCapacitySessions.length;
      
      const lowCapacityAccuracy = lowCapacitySessions.reduce((sum, s) => 
        sum + (s.correctAnswers / Math.max(1, s.questionsAnswered)), 0
      ) / lowCapacitySessions.length;
  ...
  describe('Response Time Research Validation', () => {
    ...
    it('should show anxiety effects on response times', () => {
      ...
      const anxiousSessions = largeDataset.sessions.filter(s =>
        anxiousProfiles.some(p => p.studentId === s.studentId)
      );
      const calmSessions = largeDataset.sessions.filter(s =>
  ...
  describe('Help-Seeking Behavior Research', () => {
    ...
    it('should show individual differences in help-seeking behavior', () => {
      ...
      const quickSeekerSessions = sessions.filter(s =>
        quickHelpSeekers.some(p => p.studentId === s.studentId)
      );
      const slowSeekerSessions = sessions.filter(s =>
        slowHelpSeekers.some(p => p.studentId === s.studentId)
      );
  ...
  describe('Statistical Validity Checks', () => {
    it('should generate data with appropriate statistical properties', () => {
      ...
      const withinOneStd = learningRates.filter(rate => 
        Math.abs(rate - mean) <= stdDev
      ).length;
