...
export class ContentAnalyzer {
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        return parsed.filter((c: any) => 
          c.concept && typeof c.importance === 'number' && c.category
        );
      }
    } catch (error) {
  ...
  async identifyLearningObjectives(text: string, _metadata: any): Promise<LearningObjective[]> {
    ...
    const actionVerbPatterns = /\b(?:will\s+)?(understand|explain|apply|analyze|evaluate|create|solve|design|implement|demonstrate)\b[^.,;]+/gi;
  ...
  private identifyContentSections(text: string, headings: Array<{ level: number; text: string }>): Array<{ heading?: string; content: string; start: number }> {
      ...
      for (let i = 0; i < headings.length; i++) {
        ...
        if (headingIndex !== -1) {
          const nextHeadingIndex = i < headings.length - 1 
            ? text.indexOf(headings[i + 1].text, headingIndex + 1)
            : text.length;
  ...
  private assessSectionComplexity(text: string): {
    ...
    return {
      ...
      hasEvaluativeContent: /\b(advantage|disadvantage|benefit|drawback|strength|weakness|pros?|cons?|better|worse|effective|ineffective)\b/.test(lower),
  ...
  private extractKeywords(text: string): string[] {
    const stopWords = new Set(['the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'from', 'as', 'is', 'was', 'are', 'were', 'be', 'been', 'being', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'may', 'might', 'must', 'can', 'shall']);
