...
export class ResponseFormatter {
  ...
  static sanitizeContent(content: string): string {
    // Basic sanitization - remove dangerous HTML but preserve safe markdown
    ...
    // Use DOMPurify for additional sanitization if HTML is present
    if (/<[^>]+>/.test(sanitized)) {
  ...
  static markdownToHtml(content: string): string {
    ...
    html = html.replace(this.CODE_BLOCK_PATTERN, (match, lang, code) => {
    ...
    html = html.replace(this.LATEX_PATTERN, (match, block, inline) => {
      ...
      latexExpressions.push(`<span class="${className}" data-latex="${this.escapeHtml(latex)}">${match}</span>`);
      return placeholder;
    });
    ...
    // 4. Headings
    html = html.replace(this.HEADING_PATTERN, (match, hashes, text) => {
  ...
  static extractPlainText(content: string): string {
    return content
      ...
      .replace(this.LATEX_PATTERN, (match, block, inline) => block || inline)
  ...
  static validateUserInput(input: string): { isValid: boolean; sanitized: string; issues: string[] } {
    ...
    if (/<script|<iframe|javascript:|on\w+\s*=/i.test(input)) {
      ...
      sanitized = this.sanitizeContent(input);
    }
