...
export class Tokenizer {
   ...
   */
  static estimateConversationTokens(messages: Array<{ role: string; content: string }>, modelName?: string): number {
    let totalTokens = 0;
   ...
   */
  static fitsWithinLimit(text: string, maxTokens: number, modelName?: string, buffer: number = 100): boolean {
   ...
   */
  static truncateToTokenLimit(text: string, maxTokens: number, modelName?: string, preserveEnd: boolean = false): string {
    const estimatedTokens = this.estimateTokens(text, modelName);
   ...
   */
  static splitIntoChunks(text: string, maxTokensPerChunk: number, modelName?: string, overlap: number = 50): string[] {
    const chunks: string[] = [];
    const charsPerToken = this.MODEL_CHAR_PER_TOKEN[modelName || 'default'] || 4;
  ...
  static calculateCost(
    ...
    const modelPricing: Record<string, number> = {
      '@cf/meta/llama-3.1-8b-instruct': 0.1,
      '@cf/meta/llama-3.1-70b-instruct': 0.5,
