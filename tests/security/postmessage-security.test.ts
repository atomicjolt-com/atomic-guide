...
describe('PostMessage Security Fixes', () => {
  ...
  describe('HMAC Signature Security', () => {
    it('should throw error when LTI shared secret is missing', async () => {
      ...
      await expect(extractor['generateMessageSignature'](message)).rejects.toThrow(
        'LTI shared secret not found. Cannot authenticate message.'
      );
    });
  ...
  describe('Target Origin Security', () => {
    it('should throw error when no valid origin can be determined', () => {
      ...
      expect(() => extractor['getTargetOrigin']()).toThrow(
        'Unable to determine secure target origin for postMessage. Please configure LMS origin.'
      );
    });
