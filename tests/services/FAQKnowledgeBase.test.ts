...
describe('FAQKnowledgeBase', () => {
  ...
  describe('searchSimilarFAQs', () => {
    ...
    it('should return cached results when available', async () => {
      ...
      mockKvCache.get.mockResolvedValue(JSON.stringify({
        data: cachedResults,
        timestamp: Date.now()
      }));
  ...
  describe('addFAQ', () => {
    it('should add new FAQ with vector embedding', async () => {
      ...
      const result = await faqService.addFAQ(
        'What is mitosis?',
    ...
    it('should handle database errors during FAQ creation', async () => {
      ...
      await expect(faqService.addFAQ(
        'Test question',
        'Test answer',
        'tenant-1'
      )).rejects.toThrow('Failed to add FAQ entry');
    });
  });
  ...
  describe('deleteFAQ', () => {
    it('should delete FAQ from both vector and database', async () => {
      ...
      expect(mockDB.prepare).toHaveBeenCalledWith(
        expect.stringContaining('DELETE FROM faq_entries')
      );
    });
    ...
    it('should handle missing FAQ during deletion', async () => {
      ...
      await expect(faqService.deleteFAQ('nonexistent', 'tenant-1'))
  ...
  describe('filtering and relevance', () => {
    it('should filter FAQs by similarity threshold', async () => {
      const mockMatches = [
        ...
        { id: 'faq_2', score: 0.60 }, // Below threshold  
