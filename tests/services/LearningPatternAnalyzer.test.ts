  ...
  describe('Engagement Decline Detection', () => {
    it('should detect declining engagement from response patterns', async () => {
      const messages = [
        ...
        { role: 'assistant', content: "Quantum entanglement is when particles become connected...", timestamp: new Date(Date.now() - 7000) },
        { 
          role: 'user', 
          content: "ok", 
  ...
  describe('Intervention Timing Calculation', () => {
    ...
    it('should respect focused work context', async () => {
      const focusedMessages = [
        { role: 'user', content: "Let me work through this complex mathematical proof step by step to understand the underlying principles", timestamp: new Date() }
      ];

      const analysis = await analyzer.analyzeConversation(
        focusedMessages,
        undefined,
        { pageType: 'exam', topic: 'mathematics' }
      );
